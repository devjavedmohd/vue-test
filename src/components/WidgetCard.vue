<template>
  <div :class="[`widget-card w-${widget.selectedColor}`]">
    <div :class="['widget-header', widget.selectedColor]">
    ...
    <div class="widget-options">
      <div class="option">
        ...
        <div
          class="checkbox"
          :class="{ 'checkbox-selected': widget.linked }"
          @click="updateWidget('linked', !widget.linked)"
        ></div>
      </div>
      <div class="option">
        ...
        <div class="colors">
          <span
            ...
            :key="color"
            :class="['color', color, {'selected' : widget.selectedColor === color} ]"
            @click="updateWidget('selectedColor', color)"
          ></span>
        </div>
      </div>
      <div class="option">
        <div><label>Activate badge</label></div>
        <div
          class="toggle"
          :class="{ selected: widget.active }"
          @click="updateWidget('active', !widget.active)"
        ></div>
      </div>
    </div>
  </div>
...
import { Widget } from '../store/state.ts';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const updateWidget = (key: string, value: any) => {
      emit("update-widget", props.widget.id, key, value);
    };

    return { colors, updateWidget };
  },
});
</script>
...
.w-white .widget-amount-type,
.w-white .widget-action,
.w-beige .widget-amount-type,
.w-beige .widget-action {
  color: #0f7237;
}
.w-beige svg path {
  fill: #0f7237;
...
.widget.black .widget-header {
  background-color: #10cd87;
}
.widget.beige .widget-header {
  background-color: #a8bb44c7;
}

.widget-options .option label {
  width: 100%;
  color: #0f7237;
  font-family: Cabin;
  font-size: 14px;
...
.checkbox.checkbox-selected::before {
  content: "";
  width: 5px;
  height: 10px;
  border-style: solid;
